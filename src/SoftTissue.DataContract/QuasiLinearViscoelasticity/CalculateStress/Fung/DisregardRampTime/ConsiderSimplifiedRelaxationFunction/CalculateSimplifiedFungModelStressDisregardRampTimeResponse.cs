using SoftTissue.DataContract.OperationBase;

namespace SoftTissue.DataContract.QuasiLinearViscoelasticity.CalculateStress.Fung.DisregardRampTime.ConsiderSimplifiedRelaxationFunction
{
    /// <summary>
    /// It represents the response content to CalculateSimplifiedFungModelStressDisregardRampTime operation of Quasi-Linear Viscoelasticity Model.
    /// </summary>
    public class CalculateSimplifiedFungModelStressDisregardRampTimeResponse : OperationResponseBase<CalculateSimplifiedFungModelStressDisregardRampTimeResponseData> { }
}
